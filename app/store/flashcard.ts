...
const useFlashcardStore = create<FlashcardStore>()((set) => ({
  ...
  removeCard: (id) =>
    set((state) => {
      ...
      const filteredCards = cardsToRemove.filter((card) => card.id !== id);
      return {
        cards: filteredCards,
      };
    }),
  editCard: (id, payload) =>
    set((state) => {
      ...
      const existingCardId = updatedCards.findIndex((card) => card.id === id);
  ...
  resetProgress: (id) =>
    set((state) => {
      const updatedCards = [...state.cards];
      const cardToUpdateIdx = updatedCards.findIndex((card) => card.id === id);
